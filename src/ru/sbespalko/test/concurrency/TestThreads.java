...
/**
 *  ласс <code>TestRunnable</code> демонстрирует работу потоков в много€дерной
 * системе. ѕри задании числа потоков меньшим числа €дер(+hyperthreading - 1)
 * быстродействие каждого потока не уменьшаетс€. 
 * ѕри использовании hyperthreading 1 поток резервируетс€ дл€ диспетчеризации. 
 * Ќа печать выводитс€ - врем€ работы с учетом запуска/остановки процессов. 
 *  ол-во операций ++, выполненное всеми потоками за все врем€. 
 *  ол-во операций ++, выполненное всеми потоками за 1 секунду. 
 *  ол-во операций ++, выполненное 1 потоком за 1 секунду. 
 * Ќа текстах (i7 4-core*2(hyper) 3.4Ghz)получаетс€ ~1/6 - 1/10 от частоты процессора.
 * 
 * @author Bespalko
 *
 */
